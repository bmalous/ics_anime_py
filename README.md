...
For the PowerShell implementation, see [ics_anime_ps1](https://github.com/bmalous/ics_anime_ps1).
...
Cached genre loading reuses grouped episode data for completion checks and builds exclusions from saved selections. It avoids repeatedly scanning the full episode cache for each show. Uncached seasons and explicit refreshes still require network time.
...
- Space toggles a show immediately, updating only that show's selection instead of rebuilding selections across every cached season.
...
| Menu pauses | Show toggles update one selection at a time. Cached genre loading reuses grouped episodes, and review navigation caches rows. An uncached season or requested refresh still takes network time. |
