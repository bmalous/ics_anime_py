...
review entries. The season picker offers last, current, next and future seasons,
with premiere-based rollover and unfinished English dubs retained in Last.
It fetches an uncached future season only when selected. Older provider-filtered
...
Requires Python 3.9+. Named timezone data (tzdata on Windows) enables provider schedules.
"""
...
GENRE_ITEMS = [
    ...
    ('Other', 'ExcludeFantasyIsekaiReincarnation'),
]
...
# ---------------------------------------------------------------------------
class Season:
    __slots__ = ('Season', 'Year', 'View')

    def __init__(self, season, year, view=''):
    ...
    def label(self):
        return ('Last - ' if self.View == 'Last' else '') + '{0} {1}'.format(self.Season, self.Year)
...
def get_livechart_entries(html_text, page_url):
    ...
    for match in _LC_ARTICLE.finditer(html_text):
        ...
        # The card's premiere and its next release are separate dates. A
        # countdown can also announce a batch, such as Magical Explorer EP1–2.
        releases = {}
        if to_int(premiere_text) > 0 and to_int(precision) >= 4:
            releases[1] = to_int(premiere_text)
        for overlay in re.finditer(r'<a\b[^>]*class="[^"]*episode-countdown[^"]*"[^>]*>(.*?)</a>', body, _LC_FLAGS):
            block = html.unescape(overlay.group(1))
            label = _lc_group(r'release-schedule-info[^>]*>(?P<v>[^<]*)', block)
            episode = re.search(r'\bEP\s*(\d+)(?:\s*[-\u2013\u2014]\s*(\d+))?', label, re.I)
            timestamp = to_int(_lc_group(r'data-timestamp="(?P<v>\d+)"', block))
            if not episode or not timestamp or re.search(r'\bdub\b', label, re.I):
                continue
            first, last = int(episode[1]), int(episode[2] or episode[1])
            if 0 < first <= last and last - first <= 1000:
                for number in range(first, last + 1):
                    releases[number] = timestamp
        ...
        entries.append(SimpleNamespace(
            ...
            Episode=1,
            Releases=[dict(episode=number, airingAt=stamp, TimingSource='LiveChart', SourceUrl=page_url + '#anime-' + live_id)
                      for number, stamp in sorted(releases.items())],
...
def resolve_livechart_airings(nodes, schedule, conflict_from=0):
    """Prefer published LiveChart releases; withdraw conflicting downstream dates."""
    preferred = {to_int(n.get('episode')): dict(n) for n in as_list((schedule or {}).get('Releases'))
                 if to_int(n.get('episode')) > 0 and to_int(n.get('airingAt')) > 0}
    conflicts = [conflict_from] if conflict_from else []
    for node in nodes:
        episode, stamp = to_int(node.get('episode')), to_int(node.get('airingAt'))
        if to_int((schedule or {}).get('Precision')) >= 3 and to_int(schedule.get('PremiereAt')) > 0:
            premiere_day = to_int(schedule['PremiereAt']) // 86400
            if (episode == 1 and stamp // 86400 != premiere_day) or (episode > 1 and stamp // 86400 < premiere_day):
                conflicts.append(1)
        for number, release in preferred.items():
            if ((episode == number and stamp != to_int(release['airingAt']))
                    or (episode > number and stamp < to_int(release['airingAt']))):
                conflicts.append(number)
    conflict_from = min(conflicts) if conflicts else 0
    result = {to_int(n['episode']): dict(n) for n in nodes
              if not conflict_from or to_int(n['episode']) < conflict_from}
    result.update(preferred)
    return list(result.values()), conflict_from
...
def get_lineup_sources(season_name, season_year, path):
    ...
    if in_ci('Crunchyroll', S.providers):
        index = get_web_page_text('https://www.crunchyroll.com/news/seasonal-lineup')
        pattern = (r'(?:https://www\.crunchyroll\.com)?(/news/seasonal-lineup/\d{4}/\d+/\d+/'
                   + re.escape(season_name.lower()) + r'-anime-' + str(season_year)
                   + r'-crunchyroll)')
        match = re.search(pattern, html.unescape(index))
        if match:
            sources['Crunchyroll'] = 'https://www.crunchyroll.com' + match.group(1)
    if not path:
        return sources
...
def parse_crunchyroll_schedules(page, source_url, season_name, year):
    """Accept explicit subtitled weekly schedules, never dub times or TBA."""
    month = {'WINTER': 1, 'SPRING': 4, 'SUMMER': 7, 'FALL': 10}[season_name.upper()]
    begin = datetime(int(year), month, 1)
    end = (datetime(int(year) + 1, 1, 1) if month == 10
           else datetime(int(year), month + 3, 1)) - timedelta(days=1)
    rules = []
    for section in re.finditer(r'<h[23]\b[^>]*>(.*?)</h[23]>(.*?)(?=<h[23]\b|$)', page, re.I | re.S):
        title = convert_from_anime_html(section.group(1))
        text = convert_from_anime_html(re.sub(r'</(?:p|div|li)>', ' ', section.group(2), flags=re.I))
        match = re.search(r'Sub Airtime:\s*(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)'
                          r'\s+at\s+(\d{1,2}):(\d{2})\s*([ap])\.?m\.?\s+PT\b', text, re.I)
        if not match or not 1 <= int(match[2]) <= 12 or int(match[3]) > 59:
            continue
        hour = int(match[2]) % 12 + (12 if match[4].lower() == 'p' else 0)
        rules.append(dict(provider='Crunchyroll', title=title, weekday=match[1],
                          time='{:02d}:{}'.format(hour, match[3]), timezone='America/Los_Angeles',
                          validfrom=begin.date().isoformat(), validto=end.date().isoformat(),
                          sourceurl=source_url))
    return rules


def english_dub_titles(page):
    titles = set()
    for section in re.finditer(r'<h[23]\b[^>]*>(.*?)</h[23]>(.*?)(?=<h[23]\b|$)', page, re.I | re.S):
        text = convert_from_anime_html(section.group(2))
        if re.search(r'Dub(?:bing)? Languages?:\s*[^.;\n]*\bEnglish\b', text, re.I):
            titles.add(convert_from_anime_html(section.group(1)).casefold())
    return titles


def provider_release_time(entry, show, providers):
    cache = getattr(S, 'browse_release_times', None)
    if cache is None:
        return uncached_provider_release_time(entry, show, providers)
    key = tuple(str(entry.get(field)) for field in
                ('MediaId', 'Episode', 'StartUtc', 'Date', 'AllDay', 'AirType', 'TimingSource', 'SourceUrl')) + tuple(providers)
    if key not in cache:
        cache[key] = uncached_provider_release_time(entry, show, providers)
    return cache[key]


def uncached_provider_release_time(entry, show, providers):
    """Earliest known release in a combined group; broadcast fallback otherwise.

    Weekly rules only apply to timed episodes within 36 hours after broadcast.
    This avoids moving delayed episodes a week.
    """
    broadcast = get_event_instant(entry)
    if entry.get('AirType') == 'dub':
        return broadcast, 'English dub: ' + str(entry.get('SourceUrl') or '')
    if entry.get('TimingSource') == 'LiveChart':
        return broadcast, 'LiveChart: ' + str(entry.get('SourceUrl') or '')
    if not broadcast or entry.get('AllDay'):
        return broadcast, ''
    if not hasattr(S, 'release_rules'):
        path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'provider-release-times.csv')
        S.release_rules = list(read_csv_rows(path, 'Provider release times')) if os.path.isfile(path) else []
    rules = list(show.get('ProviderSchedules') or []) + S.release_rules
    candidates = {}
    for rule in rules:
        provider = rule.get('provider', '')
        if not in_ci(provider, providers) or provider.lower() in candidates:
            continue
        if rule.get('anilistid'):
            if str(rule['anilistid']) != str(show.get('MediaId')):
                continue
        elif str(rule.get('title', '')).casefold() != str(show.get('Title', '')).casefold():
            continue
        try:
            zone = ZoneInfo(rule['timezone'])
            local = broadcast.astimezone(zone)
            weekday = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'].index(rule['weekday'].lower())
            day = local.date() + timedelta(days=(weekday - local.weekday()) % 7)
            if not rule['validfrom'] <= day.isoformat() <= rule['validto']:
                continue
            hour, minute = map(int, rule['time'].split(':'))
            release = datetime(day.year, day.month, day.day, hour, minute, tzinfo=zone).astimezone(UTC)
            wall = datetime(day.year, day.month, day.day, hour, minute, tzinfo=zone)
            if wall.utcoffset() != wall.replace(fold=1).utcoffset():
                continue  # ambiguous/nonexistent wall time
            if not timedelta(0) <= release - broadcast <= timedelta(hours=36):
                continue
            if release.astimezone(zone).replace(tzinfo=None) != datetime(day.year, day.month, day.day, hour, minute):
                continue  # nonexistent wall time during spring-forward
            candidates[provider.lower()] = (release, provider + ' schedule: ' + rule['sourceurl'])
        except ZoneInfoNotFoundError:
            if not getattr(S, 'timezone_warning', False):
                write_warning('Provider timezone data unavailable; install tzdata (python -m pip install tzdata). Using broadcast fallback.')
                S.timezone_warning = True
            continue
        except (KeyError, ValueError):
            continue
    return min(candidates.values(), key=lambda item: item[0]) if candidates else (broadcast, '')
...
def set_database_dirty():
    """Set-DatabaseDirty."""
    S.db_dirty = True
    S.db_revision = getattr(S, 'db_revision', 0) + 1
...
def update_season_cache(season_name, season_year, include_aired=False):
    ...
    # Completion and premiere detection need aired episodes even when export
    # omits them. IncludePastEpisodes remains an export preference.
    query = get_season_media_query(include_aired=True)
    media_items = []
    page_number = 1
    while True:
    ...
    provider_schedules = parse_crunchyroll_schedules(
        source_text.get('Crunchyroll', ''), sources.get('Crunchyroll', ''), season_name, season_year)
    dub_titles = english_dub_titles(source_text.get('Crunchyroll', ''))
    ...
    for live in live_entries:
        ...
        if media is not None:
            ...
            media['LiveChartSchedule'] = dict(PremiereAt=live.AiringAt, Precision=live.Precision,
                                             Releases=live.Releases, SourceUrl=live.Url)
        ...
        media_items.append({
            ...
            'LiveChartSchedule': dict(PremiereAt=live.AiringAt, Precision=live.Precision,
                                      Releases=live.Releases, SourceUrl=live.Url),
    ...
    for media in media_items:
        ...
        previous_show = ci_get(S.db['Shows'], media_id) or {}
        schedule = media.get('LiveChartSchedule') or previous_show.get('LiveChartSchedule') or {}
        if media.get('LiveChartSchedule') and previous_show.get('LiveChartSchedule'):
            # Keep earlier confirmed episode dates when the card advances.
            old_schedule = previous_show['LiveChartSchedule']
            if old_schedule.get('PremiereAt') == schedule.get('PremiereAt'):
                releases = {str(n['episode']): n for n in as_list(old_schedule.get('Releases'))}
                releases.update({str(n['episode']): n for n in as_list(schedule.get('Releases'))})
                schedule = dict(schedule, Releases=list(releases.values()))
        ...
        shows[media_id] = {
            ...
            'EnglishDub': (ci_get(S.db['Shows'], media_id) or {}).get('EnglishDub', {}),
            ...
            'ProviderSchedules': [rule for rule in provider_schedules
                                  if rule['title'].casefold() == title.casefold()],
        }
        if title.casefold() in dub_titles:
            dub = dict(shows[media_id].get('EnglishDub') or {})
            dub['Announced'] = True
            shows[media_id]['EnglishDub'] = dub

        duration = to_int(media.get('duration'), 0)
        if duration <= 0:
            duration = 30
        # A long series can have more than one hundred past episodes. Preserve
        # its upcoming schedule alongside the page used for lifecycle dates.
        airings = list({str(node.get('episode')): node
                        for node in (as_list(jget(media, 'airingSchedule', 'nodes'))
                                     + as_list(jget(media, 'upcomingSchedule', 'nodes')))
                        if isinstance(node, dict)}.values())
        airings, conflict_from = resolve_livechart_airings(airings, schedule, to_int(previous_show.get('ScheduleConflictFrom')))
        shows[media_id]['ScheduleConflictFrom'] = conflict_from
        if to_int(schedule.get('PremiereAt')) > 0 and to_int(schedule.get('Precision')) >= 3:
            premiere = datetime.fromtimestamp(to_int(schedule['PremiereAt']), UTC)
            media['startDate'] = dict(year=premiere.year, month=premiere.month, day=premiere.day)
        for airing in airings:
            ...
            events.append({
                ...
                'Sequence': sequence,
                'TimingSource': airing.get('TimingSource', 'AniList'),
                'SourceUrl': airing.get('SourceUrl', media.get('siteUrl', '')),
            })
    ...
    # Cached carryovers must not resurrect superseded timestamps from another
    # season. The current season below supplies the newly resolved events.
    for cached in S.db['Seasons'].values():
        cached['Events'] = [entry for entry in as_list(cached.get('Events'))
                            if release_is_allowed(entry, ci_get(S.db['Shows'], str(entry.get('MediaId'))) or {})]
...
def get_season_data(season_name, season_year, force_refresh=False):
    ...
    needs_schedule = bool(cached and not str(cached.get('UpdatedUtc', '')).startswith(datetime.now(UTC).strftime('%Y-%m-%d'))
                          and any((ci_get(S.db['Shows'], i) or {}).get('ScheduleConflictFrom')
                                  for i in get_season_show_ids(cached)))
    if (cached is not None and cached.get('ProviderCacheComplete') and cached.get('SchedulePriorityVersion') == 1
            and not force_refresh and not needs_aired and not needs_schedule):
...
def get_season_show_ids(season_data):
    ...
    # Membership comes from the source lineup. A September premiere can belong
    # to Fall; a continuing Summer show stays in Summer's Last view.
    return list(ids.keys())


def get_season_show_view(period, season_data, category_filter='All'):
    now = datetime.now(UTC)
    revision = getattr(S, 'db_revision', 0)
    cache = getattr(S, 'show_view_cache', None)
    if not cache or cache['db'] is not S.db or cache['revision'] != revision:
        cache = S.show_view_cache = dict(db=S.db, revision=revision, entries={})
    key = (period.key(), period.View, category_filter, tuple(S.db['EnabledProviders']))
    cached = cache['entries'].get(key)
    if cached and now < cached['expires']:
        return cached['rows']
    previous = getattr(S, 'browse_release_times', None)
    S.browse_release_times = {}
    try:
        if period.View == 'Last':
            season_data = season_view_data(period)
        rows = get_season_shows(season_data, category_filter, period.View, now)
        expires = (now + timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
        instants = [get_event_instant(entry) for entry in cached_events()]
        instants += [pair[0] for pair in S.browse_release_times.values()]
        instants += [row.LifecycleRefreshAt for row in rows]
        expires = min([expires] + [instant for instant in instants if instant and instant > now])
        cache['entries'][key] = dict(rows=rows, expires=expires)
        return rows
    finally:
        S.browse_release_times = previous


def get_season_shows(season_data, category_filter='All', view='', now=None):
    ...
    events_by_show = {media_id: [entry for entry in entries if not entry.get('AllDay')]
                      for media_id, entries in by_show.items()}
    result = []
    for media_id, entries in by_show.items():
        show = ci_get(S.db['Shows'], media_id)
        if ((category_filter == 'FantasyIsekaiReincarnation' and not show.get('IsFantasyGroup'))
                or (category_filter == 'ExcludeFantasyIsekaiReincarnation' and show.get('IsFantasyGroup'))):
            continue
        finished = test_show_finished(media_id, now=now_utc, events_by_show=events_by_show)
        if not test_show_available(show) or (finished and view != 'Last'):
            continue
        selectable = not finished or dub_pending(show, now_utc)
        ended = show_ended_at(media_id, now_utc, events_by_show) if view == 'Last' and not selectable else None
        entries = entries + as_list(jget(show, 'EnglishDub', 'Events'))
        recent = [entry for entry in entries if (get_event_instant(entry) or datetime.min.replace(tzinfo=UTC))
                  >= now_utc - timedelta(hours=36)]
        instants = [(entry, provider_release_time(entry, show, enabled_show_providers(show))[0]) for entry in recent]
        ...
        refresh_at = (now_utc + timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
        if view == 'Last':
            for entry in recent:
                instant = provider_release_time(entry, show, list(show.get('Providers', {})))[0]
                if instant and now_utc < instant < refresh_at:
                    refresh_at = instant
        result.append(SimpleNamespace(
            ...
            LastView=(view == 'Last'),
            LifecycleRefreshAt=refresh_at,
            EndedLabel=('(Ended {}) '.format(ended.astimezone().strftime('%m/%d/%y')
                        if ended else 'date unknown') if not selectable else ''),
        ))
...
def release_is_allowed(entry, show):
    if entry.get('AirType') == 'dub':
        return True
    number = to_int(entry.get('Episode'))
    schedule = show.get('LiveChartSchedule') or {}
    for node in as_list(schedule.get('Releases')):
        if to_int(node.get('episode')) == number:
            return (entry.get('TimingSource') == 'LiveChart'
                    and get_event_instant(entry) == datetime.fromtimestamp(to_int(node['airingAt']), UTC))
    cutoff = to_int(show.get('ScheduleConflictFrom'))
    if number and cutoff and number >= cutoff:
        return False
    if entry.get('Episode') == 'release' and schedule.get('Releases'):
        return False
    return True
...
def test_show_finished(media_id, now=None, events_by_show=None):
    ...
    instants = [(entry, provider_release_time(entry, show, list(show.get('Providers', {})))[0]) for entry in events]
    ...
    if final:
        return max(final) <= now
    if show.get('ScheduleConflictFrom'):
        return False  # AniList's end date belongs to the disputed schedule.
...
def previous_season(period):
    names = ['WINTER', 'SPRING', 'SUMMER', 'FALL']
    index = names.index(period.Season)
    return Season(names[(index - 1) % 4], period.Year - (index == 0))


def season_number(period):
    return period.Year * 4 + ['WINTER', 'SPRING', 'SUMMER', 'FALL'].index(period.Season)


def season_started(period, now):
    data = ci_get(S.db['Seasons'], period.key()) or {}
    for entry in as_list(data.get('Events')):
        if str(entry.get('Episode')) != '1' or entry.get('AllDay'):
            continue
        instant = get_event_instant(entry)
        show = ci_get(S.db['Shows'], str(entry.get('MediaId'))) or {}
        if instant and provider_release_time(entry, show, list(show.get('Providers', {})))[0] <= now:
            return True
    return False


def season_context(current, now=None):
    """Resolve labels by premieres, including premieres before a quarter boundary."""
    now = now or datetime.now(UTC)
    prior = previous_season(current)
    data = ci_get(S.db['Seasons'], current.key())
    # Only hold the calendar back when a precise upcoming premiere is known.
    # A missing/partial cache is not evidence that a season has not started.
    has_premiere = any(str(e.get('Episode')) == '1' and not e.get('AllDay')
                       and get_event_instant(e) for e in as_list((data or {}).get('Events')))
    if has_premiere and ci_get(S.db['Seasons'], prior.key()) and not season_started(current, now):
        current = prior
    while season_started(get_next_anime_season(current), now):
        current = get_next_anime_season(current)
    index = {}
    for entry in cached_events():
        if not entry.get('AllDay'):
            index.setdefault(str(entry.get('MediaId')), []).append(entry)
    ids = get_season_show_ids(ci_get(S.db['Seasons'], current.key()))
    ended = any(test_show_finished(media_id, now, index) for media_id in ids)
    last = current if ended else previous_season(current)
    if not ended and not ci_get(S.db['Seasons'], last.key()):
        last = None
    return current, last, index


def season_view_data(period, now=None):
    """Build a view without moving stored events or changing saved selections."""
    now = now or datetime.now(UTC)
    data = ci_get(S.db['Seasons'], period.key()) or {}
    if period.View != 'Last':
        return data
    current, _, index = season_context(get_anime_season(now), now)
    ids = set()
    for cached in S.db['Seasons'].values():
        source = Season(cached['Season'], cached['Year'])
        if season_number(source) > season_number(period):
            continue
        for media_id in get_season_show_ids(cached):
            if source == period and source != current:
                ids.add(media_id)
                continue
            finished = test_show_finished(media_id, now, index)
            show = ci_get(S.db['Shows'], media_id) or {}
            if source == period:
                if source != current or finished:
                    ids.add(media_id)
            elif not finished or dub_pending(show, now):
                ids.add(media_id)
    return dict(data, ShowIds=sorted(ids), Events=[])


def dub_pending(show, now):
    dub = show.get('EnglishDub') or {}
    if not dub.get('Announced'):
        return False
    events = [(entry, get_event_instant(entry)) for entry in as_list(dub.get('Events'))]
    if any(instant and instant > now for _, instant in events):
        return True
    total = to_int(dub.get('TotalEpisodes')) or to_int(show.get('TotalEpisodes'))
    return not any(total > 0 and to_int(entry.get('Episode')) == total
                   and instant and instant <= now for entry, instant in events)


def show_ended_at(media_id, now, index):
    show = ci_get(S.db['Shows'], media_id) or {}
    dates = [provider_release_time(entry, show, list(show.get('Providers', {})))[0]
             for entry in index.get(media_id, [])]
    dates += [get_event_instant(entry) for entry in as_list(jget(show, 'EnglishDub', 'Events'))]
    dates = [instant for instant in dates if instant and instant <= now]
    end = show.get('EndDate')
    if isinstance(end, str):
        instant = convert_from_anime_date(end, date_only=True)
        if instant and instant <= now:
            # A date-only value must retain its date in the user's timezone.
            dates.append(instant.replace(tzinfo=None).astimezone(UTC))
    return max(dates) if dates else None


def get_season_menu_items(current, now=None):
    current, last, _ = season_context(current, now)
    nxt = get_next_anime_season(current)
    future = get_next_anime_season(nxt)
    items = []
    if last:
        items.append(MenuItem('Last - ' + last.label(), Season(last.Season, last.Year, 'Last')))
    items += [
        MenuItem('Current - {0}'.format(current.label()), Season(current.Season, current.Year, 'Current')),
...
def update_english_dubs():
    """Fetch the small corrections index, then schedules for known English dubs.

    Keep the last successful response on failure. Absence from this source does
    not establish that no dub exists. Announced dubs with no finale stay active.
    """
    try:
        raw = get_web_page_text('https://tsuzuki.top/api/v1/overrides')
        payload = json.loads(raw)
        if payload.get('ok') is not True or not isinstance(payload.get('shows'), dict):
            return
    except (ValueError, TypeError):
        return
    today = datetime.now(UTC).strftime('%Y-%m-%d')
    for media_id, show in S.db['Shows'].items():
        correction = payload['shows'].get(media_id) or {}
        old = show.get('EnglishDub') or {}
        if not correction.get('dub') and not old.get('Announced'):
            continue
        if to_int(media_id) <= 0 or str(old.get('UpdatedUtc', '')).startswith(today):
            continue
        url = 'https://tsuzuki.top/api/v1/anime/' + media_id
        try:
            result = json.loads(get_web_page_text(url))
            if result.get('ok') is not True or not isinstance(result.get('episodes'), list):
                continue
            events = []
            for episode in result['episodes']:
                if (episode.get('airType') != 'dub' or episode.get('isBreak')
                        or not episode.get('airingAt') or episode.get('estimated')
                        or episode.get('exact') is not True):
                    continue
                events.append(dict(MediaId=media_id, Episode=str(episode['episode']),
                    AirType='dub', Provider=episode.get('platform'), AllDay=False,
                    StartUtc=convert_to_utc_stamp(datetime.fromtimestamp(episode['airingAt'], UTC)),
                    DurationMinutes=to_int(jget(result, 'anime', 'duration')) or 30,
                    Sequence=0, SourceUrl=url))
            # The endpoint returns the full schedule; a successful replacement
            # also removes a time that was withdrawn or changed to a break.
            show['EnglishDub'] = dict(Announced=True, Events=events,
                TotalEpisodes=to_int(jget(result, 'anime', 'episodes')) or to_int(old.get('TotalEpisodes')),
                SourceUrl=url, UpdatedUtc=round_trip_stamp(datetime.now(UTC)))
            set_database_dirty()
...
def prepare_season_menu(current):
    """Refresh lifecycle metadata once a day, outside the key handling loop."""
    today = datetime.now(UTC).strftime('%Y-%m-%d')
    if getattr(S, 'season_menu_checked', None) == today:
        return
    S.season_menu_checked = today
    periods = [previous_season(current), current, get_next_anime_season(current)]
    # Retain still-running titles older than Last across subsequent rollovers.
    for record in list(S.db['Seasons'].values()):
        period = Season(record['Season'], record['Year'])
        if season_number(period) < season_number(periods[0]):
            if any(not test_show_finished(i) or dub_pending(S.db['Shows'].get(i, {}), datetime.now(UTC))
                   for i in get_season_show_ids(record)):
                periods.append(period)
    for period in periods:
        cached = ci_get(S.db['Seasons'], period.key())
        if (cached and cached.get('IncludesAired') and cached.get('SchedulePriorityVersion') == 1
                and str(cached.get('UpdatedUtc', '')).startswith(today)):
            continue
        try:
            update_season_cache(period.Season, period.Year, include_aired=True)
        except Exception as error:
            write_warning('Could not refresh {}: {}. Using cached data.'.format(period.label(), error))
    update_english_dubs()
...
def show_show_details(show, excluded):
    ...
    while True:
        ...
        write_host('--------', 'DarkCyan')
        if getattr(show, 'EndedLabel', ''):
            write_host(show.EndedLabel.strip(), 'DarkGray')
...
def save_anime_exclusion(excluded, mark_pending=False, media_id=None):
    """Save-AnimeExclusion.
    ...
    if media_id is not None:
        ...
        selected = {str(value) for value in S.db['Selections']}
        if media_id in excluded:
            selected.discard(media_id)
        else:
            selected.add(media_id)
        S.db['Selections'] = sorted(selected, key=ps_sort_key)
    else:
        all_ids = {}
        for season in S.db['Seasons'].values():
            for show_id in get_season_show_ids(season):
                all_ids[str(show_id)] = True
        S.db['Selections'] = sorted([show_id for show_id in all_ids
                                     if show_id not in excluded], key=ps_sort_key)
    update_selection_history()
...
def refresh_last_rows(shows, now=None):
    """Reuse a row until an airing or date-only completion can change it."""
    now = now or datetime.now(UTC)
    due = [row for row in shows if getattr(row, 'LastView', False)
           and (getattr(row, 'LifecycleRefreshAt', None) is None or now >= row.LifecycleRefreshAt)]
    if not due:
        return
    refreshed = {row.MediaId: row for row in get_season_shows(
        {'ShowIds': [row.MediaId for row in due]}, view='Last', now=now)}
    for row in due:
        if row.MediaId in refreshed:
            row.__dict__.update(refreshed[row.MediaId].__dict__)
...
def show_toggle_list(shows, excluded, applied_excluded, show_excluded, title):
    ...
    search = ''
    frame = {}
    titles = {show.MediaId: format_anime_title(show.Title, show.Providers) for show in shows}
    while True:
        refresh_last_rows(shows)
        listing = [show for show in shows
                   if show_excluded is None or (show.MediaId in applied_excluded) == show_excluded]
        if search:
            needle = search.lower()
        ...
        for index in range(top, end):
            ...
            if not getattr(listing[index], 'Selectable', True):
                mark = '[-] '
            color = 'Yellow' if index == selected else 'Gray'
            lines.append((' {0} {1}{2}'.format(
                cursor, mark, titles[listing[index].MediaId]),
                color))
        ...
        if key == 'UpArrow':
            ...
            refresh_last_rows([show])
            if not getattr(show, 'Selectable', True):
                continue
            if show.MediaId in excluded:
                excluded.pop(show.MediaId, None)
            else:
                excluded[show.MediaId] = show.Title
            save_anime_exclusion(excluded, mark_pending=True, media_id=show.MediaId)
        elif key == 'C':
            search = ''
            selected = 0
        elif key == 'Q':
            ...
            refresh_last_rows([listing[selected]])
...
def get_week_airings(now=None, tz=None):
    """Selected episodes from today through the end of the next six local days.
    ...
    for entry in cached_events():
        ...
        if entry.get('AllDay'):
            ...
            instant, _ = provider_release_time(entry, show, enabled_show_providers(show))
            if instant is None:
                continue
...
def new_calendar_event(entry, show):
    ...
    if entry.get('AirType') == 'dub' and entry.get('Provider'):
        show_providers = [name for name in show_providers if name.casefold() == entry['Provider'].casefold()]
    ...
    results = []
    for group in groups:
        release, timing_source = provider_release_time(entry, show, group)
        ...
        if not entry.get('AllDay'):
            lines.append('Release time: ' + (timing_source or 'broadcast fallback; provider time unavailable.'))
            if len(group) > 1 and timing_source:
                lines.append('Combined entry uses the earliest known provider release in this group.')
        ...
        results.append(SimpleNamespace(
            ...
            Summary='{0}{1}{2}'.format(format_anime_title(show.get('Title'), group), episode_label,
                                      ' (English Dub)' if entry.get('AirType') == 'dub' else ''),
            Description=NEWLINE.join(lines),
            Url=primary_url,
            AllDay=bool(entry.get('AllDay')),
            Start=release,
            Minutes=to_int(entry.get('DurationMinutes'), 0),
            Sequence=to_int(entry.get('Sequence'), 0) + (1 if timing_source else 0),
        ))
    return results
...
def export_anime_calendar(path, excluded):
    ...
    calendar_events = []
    dub_season = {'Events': [entry for show in S.db['Shows'].values()
                             for entry in as_list(jget(show, 'EnglishDub', 'Events'))]}
    for season in list(S.db['Seasons'].values()) + [dub_season]:
        for entry in as_list(season.get('Events')):
            ...
            if show is None:
                continue
            if not release_is_allowed(entry, show):
                continue
            calendar_events.extend(event for event in new_calendar_event(entry, show)
                                   if event.Start and event.Start >= retention_start)
    ...
    # Provider corrections keep the UID and advance the existing revision.
    previous = {}
    if os.path.isfile(path):
        with open(path, encoding='utf-8-sig', errors='replace') as handle:
            for match in _VEVENT_BLOCK.finditer(handle.read()):
                block = match.group(0)
                uid = _UID_IN_BLOCK.search(block)
                seq = re.search(r'^SEQUENCE:(\d+)', block, re.M)
                if uid:
                    previous[get_canonical_event_uid(uid=uid.group(1))] = (
                        get_event_start_from_ics_block(block), int(seq[1]) if seq else 0)
    for event in calendar_events:
        if event.Uid in previous:
            old_start, old_sequence = previous[event.Uid]
            event.Sequence = max(event.Sequence, old_sequence + int(old_start != event.Start))
    uids = set()
    event_keys = set()
    ...
    for existing_path in merge_paths:
        ...
        for match in _VEVENT_BLOCK.finditer(raw):
            ...
            if media_match:
                ...
                show = ci_get(S.db['Shows'], media_id) or {}
                episode_match = re.match(r'^-?\d+-(\d+|release)-(.+)@', uid)
                if episode_match and not episode_match[2].endswith('Dub'):
                    cutoff = to_int(show.get('ScheduleConflictFrom'))
                    if ((cutoff and to_int(episode_match[1]) >= cutoff)
                            or (episode_match[1] == 'release' and jget(show, 'LiveChartSchedule', 'Releases'))):
                        continue
...
def sync_exclusion_state():
    ...
    for media_id, show in S.db['Shows'].items():
        ...
        if key not in selected_ids:
            mapping[key] = str((show or {}).get('Title') or '')
    return mapping
...
def get_desktop_directory():
    """Resolve the user's Windows, macOS, or Linux desktop directory."""
    if os.name == 'nt':
        try:
            import winreg
            key = winreg.OpenKey(
    ...
    if sys.platform.startswith('linux'):
        config = os.path.join(os.environ.get('XDG_CONFIG_HOME') or
                              os.path.join(user_directory, '.config'), 'user-dirs.dirs')
        try:
            with open(config, encoding='utf-8') as handle:
                for line in handle:
                    match = re.match(r'^\s*XDG_DESKTOP_DIR="(.*)"\s*$', line)
                    if match:
                        value = match.group(1).replace('${HOME}', user_directory).replace('$HOME', user_directory)
...
def run(argv):
    ...
    S.db = import_anime_database(S.db_path, launch_directory)
    resolve_duplicate_shows()
    ...
    export_return_state = 'MAIN'

    while not exit_requested:
        if state in ('MAIN', 'MANAGE', 'PROVIDERS', 'WEEK', 'UPDATES', 'SEASONPICK', 'GENRE'):
        ...
        if state == 'MAIN':
            ...
            prepare_season_menu(current_season)
            ...
            shows = get_season_show_view(target_season, season_data, S.category_filter)
            ...
            if args.OutputPath:
                export_path = resolve_output_path(args.OutputPath, launch_directory)
                state = 'EXPORT'
                continue
            folder = select_calendar_folder(launch_directory)
            ...
            if result.Total == 0:
                ...
                if not as_list(S.db['Selections']) and S.db['Shows']:
                    ...
                    write_warning('No titles currently have both a supported streaming-provider '
                                  ...
                                  'later.')
            write_warning('Provider schedules are preferred; entries marked broadcast fallback '
                          'may precede streaming availability. Schedules vary by region.')
            ...
            after = show_key_menu('Calendar created', [
                MenuItem('Back to previous menu', 'BACK'),
                MenuItem('Quit', 'QUIT'),
            ])
            if after == 'BACK':
                state = export_return_state
