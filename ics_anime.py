...
def format_data_size(count):
    ...
    for suffix, size in (('GB', 1024 ** 3), ('MB', 1024 ** 2), ('KB', 1024)):
        if count >= size:
            return '{0:.2f} {1}'.format(float(count) / size, suffix)
    return '{0} bytes'.format(count)
...
def get_canonical_event_uid(uid=None, event_id=None):
    ...
    if match:
        return '{0}{1}'.format(match.group('id'), UID_SUFFIX)
    return value
...
def import_anime_database(path, default_directory):
    ...
    for key in ('Selections', 'AppliedSelections', 'KnownShows', 'PendingUpdates', 'EnabledProviders'):
        if ci_contains(loaded, key):
    ...
    for key in ('Shows', 'Seasons', 'ProviderDirectory', 'ScrapeBytesBySource'):
...
def update_season_cache(season_name, season_year, include_aired=False):
    ...
    if S.args.ProviderOverridesPath:
        for row in read_csv_rows(S.args.ProviderOverridesPath, 'Provider override list'):
            if row.get('anilistid') and in_ci(row.get('provider'), S.providers):
    ...
    for media in media_items:
        ...
        for link in as_list(media.get('externalLinks')):
            ...
            if (service and (S.args.AllDiscoveredProviders or in_ci(service, S.providers))
                    and not ci_contains(service_links, service)):
        ...
        if 'LiveChartProviderLinks' in media:
            for service, url in (media.get('LiveChartProviderLinks') or {}).items():
                ...
                if ((S.args.AllDiscoveredProviders or in_ci(service, S.providers))
                        and not ci_contains(service_links, service)):
...
def get_season_data(season_name, season_year, force_refresh=False):
    ...
    if cached is not None and not force_refresh and not needs_aired:
...
def get_season_shows(season_data, category_filter='All'):
    ...
    for media_id, entries in by_show.items():
        ...
        result.append(SimpleNamespace(
            ...
            Providers=list((show.get('Providers') or {}).keys()),
...
def show_key_menu(title, items, selected=0, back=False):
    """Show-KeyMenu."""
    if not items:
        return None
    while True:
        clear_host()
        write_host(title, 'Cyan')
        write_host('=' * len(title), 'DarkCyan')
        for index, item in enumerate(items):
            marker = '>' if index == selected else ' '
            color = 'Yellow' if index == selected else 'Gray'
            write_host(' {0} {1}'.format(marker, item.Label), color)
        write_host('')
        write_host('Up/Down Move   Right/Enter/Space Select   Left Back   Q Quit', 'DarkGray')
        key = read_menu_key()
        if key == 'Q':
...
def remove_disabled_provider_data():
    ...
    for media_id in list(S.db['Shows'].keys()):
        show = S.db['Shows'][media_id]
        providers = show.get('Providers') or {}
        for name in list(providers.keys()):
            if str(name).lower() not in enabled:
                del providers[name]
                changed = True
        if providers:
            continue
        del S.db['Shows'][media_id]
        S.db['Selections'] = [value for value in as_list(S.db['Selections'])
                              if str(value) != str(media_id)]
        S.db['AppliedSelections'] = [value for value in as_list(S.db['AppliedSelections'])
                                     if str(value) != str(media_id)]
        S.db['KnownShows'] = [value for value in as_list(S.db['KnownShows'])
                              if str(value) != str(media_id)]
        for season in S.db['Seasons'].values():
            season['ShowIds'] = [value for value in as_list(season.get('ShowIds'))
                                 if str(value) != str(media_id)]
            season['Events'] = [entry for entry in as_list(season.get('Events'))
                                if str(entry.get('MediaId')) != str(media_id)]
    if changed:
...
def refresh_provider_seasons(current):
    ...
    cached_shows = 0
    for period in (current, get_next_anime_season(current)):
...
def show_provider_details(provider, current):
    ...
    while True:
        clear_host()
        write_host(provider['Name'], 'Cyan')
        if provider.get('Url'):
            esc = '\x1b'
            write_host('\nWebsite: {0}]8;;{1}{0}\\{1}{0}]8;;{0}\\'
                       .format(esc, provider['Url']), 'Blue')
        else:
            write_host('\nWebsite: Not available', 'DarkGray')
        write_host('\nPopular currently airing shows', 'DarkCyan')
        if not top:
            write_host('  Not available')
        else:
            for index, show in enumerate(top):
                name = format_anime_title(show.Title, show.Providers)
                write_host(' {0} {1}'.format('>' if index == selected else ' ', name),
                           'Yellow' if index == selected else 'Gray')
        write_host('\nUp/Down Move   Right Show details   O Open website   '
                   'Left Back   Q Quit', 'DarkGray')
        key = read_menu_key()
    ...
    while True:
        ...
        for index, item in enumerate(listing):
            if item.IsUpdate:
                ...
                mark = '[ ]'
            write_host(' {0} {1} {2}'.format('>' if index == selected else ' ', mark, item.Name),
                       'Yellow' if index == selected else 'Gray')
        if not providers:
            write_host('\nSelect Update once to discover providers reported by AniList and '
                       'LiveChart.', 'DarkGray')
        write_host('\nUp/Down Move   Space Toggle/Update   Right Provider details   '
                   'Left Back   Q Quit', 'DarkGray')
        key = read_menu_key()
...
def show_toggle_list(shows, excluded, applied_excluded, show_excluded, title):
    ...
    while True:
        ...
        page_size = get_menu_page_size()

        clear_host()
        write_host(title, 'Cyan')
        write_host('=' * len(title), 'DarkCyan')
        if search:
            write_host('Filter: {0}'.format(search), 'Magenta')

        if not listing:
            write_host('')
            write_host('Nothing matches that filter.' if search else 'No shows in this list.')
            write_host('')
            write_host('/ Filter   C Clear filter   Left Back   Q Quit', 'DarkGray')
            key = read_menu_key()
            if key == 'Q':
                answer = resolve_quit_request()
                if answer != 'BACK':
        ...
        if selected < 0:
            selected = 0
        top = max(0, min(selected - int(page_size / 2), len(listing) - page_size))
        if top < 0:
            top = 0
        for index in range(top, min(top + page_size, len(listing))):
            cursor = '>' if index == selected else ' '
            mark = '[ ] ' if listing[index].MediaId in excluded else '[x] '
            color = 'Yellow' if index == selected else 'Gray'
            write_host(' {0} {1}{2}'.format(
                cursor, mark, format_anime_title(listing[index].Title, listing[index].Providers)),
                color)
        write_host('')
        write_host('{0}/{1}   Up/Down Move   PgUp/PgDn   Space Toggle   Right Details   '
                   '/ Filter   C Clear   Left Back'.format(selected + 1, len(listing)),
                   'DarkGray')
...
def show_pending_updates():
    ...
    selected = 0
    while True:
        items = sort_ci(as_list(S.db['PendingUpdates']),
                        key=lambda item: (item or {}).get('Title'))
        if not items:
            return None
        if selected >= len(items):
            selected = len(items) - 1
        clear_host()
        write_host('Updated next-season shows', 'Cyan')
        write_host('=========================', 'DarkCyan')
        for index, item in enumerate(items):
            ...
            name = format_anime_title(item.get('Title'),
                                      record.get('Providers') if record else [])
            write_host(' {0} {1} {2} - {3}'.format(cursor, mark, name, item.get('Reason')), color)
        write_host('')
        write_host('Up/Down Move   Space Add/Remove   Right Details   D Dismiss all   '
                   'Left Back', 'DarkGray')
        key = read_menu_key()
        if key == 'UpArrow':
            selected = (selected - 1 + len(items)) % len(items)
            ...
            projection = SimpleNamespace(
                ...
                Providers=list((record.get('Providers') or {}).keys()) if record else [],
                NextEvent=None,
    ...
    for existing_path in merge_paths:
        ...
        for match in _VEVENT_BLOCK.finditer(raw):
            ...
            if media_match:
                ...
                if excluded and media_id in excluded:
                    continue
                if (selected and ci_contains(S.db['Shows'], media_id)
                        and media_id not in selected):
                    continue
...
def run(argv):
    ...
    while not exit_requested:
        if state == 'MAIN':
            ...
            items.append(MenuItem('Data scraped (all time): {0}'.format(
                format_data_size(S.db.get('ScrapeBytesTotal', 0))), 'SCRAPE_TOTAL'))
            items.append(MenuItem('Providers ({0} enabled)'.format(
                len(as_list(S.db['EnabledProviders']))), 'PROVIDERS'))
            if as_list(S.db['PendingUpdates']):
                items.append(MenuItem('Review updated shows ({0})'.format(
                    len(as_list(S.db['PendingUpdates']))), 'UPDATES'))
            ...
            choice = show_key_menu('Anime Calendar', items)
            if choice is None or choice == 'QUIT':
                state = 'QUIT'
            elif choice == 'SCRAPE_TOTAL':
                state = 'MAIN'
            elif choice == 'LOAD':
